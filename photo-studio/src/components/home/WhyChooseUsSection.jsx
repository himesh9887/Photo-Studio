...
function CounterCard({ item, index }) {
  ...
  return (
    <motion.div
      ...
      className="section-shell rounded-[1.1rem] p-3.5 sm:rounded-[1.8rem] sm:p-6"
    >
      <span className="inline-flex rounded-lg p-2 sm:rounded-2xl sm:p-3" style={{ background: "color-mix(in oklab, var(--gold) 12%, transparent)", color: "var(--gold)" }}>
        <Icon size={20} />
      </span>

      <div className="mt-3 flex items-end gap-2 sm:mt-5">
        <p className="text-[1.7rem] font-semibold sm:text-5xl">
          {count}
          {item.suffix}
        </p>
      </div>
      <p className="mt-1 text-[9px] font-semibold uppercase tracking-[0.1em] text-[color:var(--muted)] sm:mt-2 sm:text-xs sm:tracking-[0.24em]">
        {item.label}
      </p>
      <h3 className="mt-3 text-lg font-semibold sm:mt-5 sm:text-2xl">{item.title}</h3>
      <p className="mt-2 text-[13px] leading-6 text-[color:var(--muted)] sm:mt-3 sm:text-base sm:leading-8">
        {item.description}
      </p>
    </motion.div>
  );
  ...
  return (
    <section className="px-4 py-12 sm:px-6 sm:py-20 lg:px-8">
      <div className="premium-container">
        ...
        <div className="mt-8 grid gap-3 sm:mt-14 sm:gap-6 md:grid-cols-2 xl:grid-cols-4">
