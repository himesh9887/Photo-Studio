  ...
  return (
    <div className="relative overflow-hidden pb-20 pt-24 sm:pt-28 md:pb-24 md:pt-32">
      ...
      <div className="premium-container">
          ...
          <div className="relative grid gap-5 lg:grid-cols-[1.05fr_0.95fr] lg:items-end">
            <div>
              ...
              <h1 className="mt-4 max-w-4xl text-[2rem] leading-[1.02] sm:text-5xl lg:text-7xl">
                Browse the gallery like a curated collection, not a random image grid.
              </h1>
              <p className="mt-4 max-w-2xl text-sm leading-7 text-(--muted) sm:mt-6 sm:text-lg sm:leading-9">
                Featured storytelling, cleaner category filters, aur stronger card hierarchy ke saath portfolio ko zyada premium aur usable banaya gaya hai.
              </p>
            </div>

            <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1 lg:justify-self-end lg:max-w-sm">
              {[
                [`${gallery.length}`, "signature stories"],
                [`${items.length}`, "currently visible"],
                ["4 styles", "shoot categories"],
              ].map(([value, label]) => (
                <div
                  key={label}
        ...
        {featuredItem ? (
          <motion.section layout className="mt-6 grid gap-4 lg:grid-cols-[1.12fr_0.88fr]">
            <motion.div
              key={featuredItem.id}
              layout
            ...
            >
              ...
              >
                <div className="relative h-72 overflow-hidden sm:h-104 lg:h-[34rem]">
                  <motion.img
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-4 sm:p-6 lg:p-8">
                    ...
                    <p className="mt-3 max-w-xl text-sm leading-7 text-white/74 sm:text-base sm:leading-8">
                      Strong hero placement gallery ko editorial landing feel deta hai aur first click naturally guide karta hai.
                    </p>
                  </div>
                </div>
              </Link>
            ...
            <div className="grid gap-4">
              <div className="rounded-[1.35rem] border border-(--line) bg-(--surface) p-4 shadow-[0_18px_36px_rgba(0,0,0,0.08)] sm:rounded-3xl sm:p-5">
                ...
                <p className="mt-3 text-sm leading-7 text-(--muted)">
                  Current filter ke hisaab se cards re-order hote hain taki strongest story top par rahe.
                </p>
              </div>
              ...
              <div className="rounded-[1.35rem] border border-(--line) bg-[linear-gradient(135deg,color-mix(in_oklab,var(--gold)_8%,transparent),color-mix(in_oklab,var(--surface)_94%,transparent))] p-4 shadow-[0_18px_36px_rgba(0,0,0,0.08)] sm:rounded-3xl sm:p-5">
                ...
                <div className="mt-4 space-y-3">
                  {[
                    "Featured first impression",
                    "Balanced support cards",
                    "Cleaner grid for deeper browsing",
                  ].map((item) => (
                    <div
                      key={item}
        ...
        {supportingItems.length ? (
          <motion.section layout className="mt-6 grid gap-4 md:grid-cols-2">
            <AnimatePresence mode="popLayout">
              {supportingItems.map((item) => (
                ...
                >
                  <Link
                    to={`/gallery/${item.id}`}
                    className="group block overflow-hidden rounded-[1.5rem] border border-(--line) bg-(--surface) shadow-[0_18px_36px_rgba(0,0,0,0.08)] sm:shadow-[0_22px_55px_rgba(0,0,0,0.12)]"
                  >
          ...
          <AnimatePresence mode="popLayout">
            {gridItems.map((item, index) => (
              ...
              >
                ...
                >
                  <div className={`overflow-hidden ${index % 3 === 0 ? "h-64 sm:h-80" : "h-56 sm:h-72"}`}>
                    <motion.img
                      src={item.image}
                      alt={item.title}
                      className="h-full w-full object-cover"
